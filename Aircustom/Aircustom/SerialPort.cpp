...
// start comm watching
//
BOOL CSerialPort::StartMonitor(void)
{
...
//
BOOL CSerialPort::ResumeMonitor(void)
{
...
//
BOOL CSerialPort::SuspendMonitor(void)
{
...
BOOL CSerialPort::StopMonitor(void)
{
...
void CSerialPort::WriteChar(CSerialPort* port)
{
	...
	if (bWrite)
	{
		...
		bResult = WriteFile(port->m_hComm,		   // Handle to COMM Port
							...
							port->m_ulWriteLen,    // Length of message to send
	...
	if (BytesSent != port->m_ulWriteLen)
	{
		TRACE("WARNING: WriteFile() error.. Bytes Sent: %d; Message Length: %d\n", BytesSent, port->m_ulWriteLen);
	}
}
...
//
void CSerialPort::WriteToPort(unsigned char* writebuf, unsigned long buflen)
{		
	assert(m_hComm != 0);

	memset(m_szWriteBuffer, 0, m_nWriteBufferSize);
	memcpy(m_szWriteBuffer, writebuf, buflen);
	m_ulWriteLen = buflen;
	// set event for write
	SetEvent(m_hWriteEvent);
}

void CSerialPort::WriteSerial(const unsigned char* buf, unsigned char len)
{
	...
	bWriteStat=WriteFile(m_hComm, buf,
...
void CSerialPort::UninitPort(void)
{
	StopMonitor();
	if (INVALID_HANDLE_VALUE != m_hComm)
	{
