...
BEGIN_MESSAGE_MAP(CTopologyXApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CTopologyXApp::OnAppAbout)
	// 基于文件的标准文档命令
...
CTopologyXApp::CTopologyXApp()
{
	...
	SetAppID(_T("TopologyX.AppID.NoVersion"));

	// TODO: 在此处添加构造代码，
	// 将所有重要的初始化放置在 InitInstance 中
}

// 唯一的一个 CTopologyXApp 对象

CTopologyXApp theApp;


// CTopologyXApp 初始化

BOOL CTopologyXApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CTopologyXDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架窗口
		RUNTIME_CLASS(CTopologyXView));
	if (!pDocTemplate)
		return FALSE;
...
int CTopologyXApp::ExitInstance()
{
	//TODO: 处理可能已添加的附加资源
	AfxOleTerm(FALSE);
...
int CTopologyXApp::CStringToChar(const CString& strSrc, CHAR *szDst, int nDstLen)
{
	int lStrCount;
	int lByteCount = -1;
	CHAR *pcTmpStr;
